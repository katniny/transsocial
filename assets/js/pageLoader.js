...
supabase.auth.onAuthStateChange(async (event, session) => {
   ...
   if (user) {
      ...
      let { data: userData, error } = await supabase
         .from("public")
         .select("theme, fontSizePref, showPrideFlag")
         .eq("id", uid)
         .single();
      ...
      if (userData.fontSizePref) {
         if (userData.fontSizePref === "normal") {
      ...
      if (!userData.theme || userData.theme === "Dark") {
         ...
         if (currentMonth === 6 && userData.showPrideFlag === "Yes" || currentMonth === 6 && userData.showPrideFlag === undefined) {
            document.getElementById("transsocialHeaderLogo").src = "/assets/imgs/PrideHeaderLogo.png";
         }
      } else if (userData.theme === "Mint (Light)") {
      ...
      } else if (userData.theme === "Mint (Dark)") {
      ...
      } else if (userData.theme === "High Contrast") {
      ...
      } else if (userData.theme === "TransSocial Classic") {
      ...
      } else if (userData.theme === "Midnight Purple") {
